...
class PersonalPropertyController extends Controller
{
    ...
    public function index($bid)
    {
        ...
        $personal_guarantor_list = PersonalGuarantorBorrower::where([
            ['borrower_id', $bid],
            ['guarantor_type', 'personal']
        ])->get();

        $corporate_guarantor_list = PersonalGuarantorBorrower::where([
            ['borrower_id', $bid],
            ['guarantor_type', 'corporate']
        ])->get();


        $vehicle = PersonalHirePurchase::where([
            ['borrower_id', $bid]
        ])->get();
...
//        land property assigned for the borrrower
        $pland_id = PersonalLandBorrower::where([
            ['borrower_id', $bid],
        ...
        $cland_id = PersonalLandBorrower::where([
            ['borrower_id', $bid],
        ...
        $pshare_id = PersonalShareBorrower::where([
            ['borrower_id', $bid],
            ['property_type', 'personal'],
        ...
        $cshare_id = PersonalShareBorrower::where([
            ['borrower_id', $bid],
            ['property_type', 'corporate']
        ])->get();
        ...
        if ($joint) {
            $jland = JointPropertyOwner::find($joint->id)->joint_land()->get();
        }else{
            $jland=[];
        }
        ...
        return view('personal_property.index', compact('jland','pland','isin', 'cland', 'pshare', 'cshare', 'bid', 'vehicle', 'personal_guarantor_list', 'corporate_guarantor_list', 'personal_property_owner', 'corporate_property_owner'));
    }
    ...
    public function copy_index($bid)
    {
        ...
        return view('personal_property.copy_index', compact('bid', 'personal_borrower', 'personal_guarantor', 'corporate_borrower', 'corporate_guarantor'));
    }
...
//        local body
        ...
        return view('personal_property.personal_create', compact('isin','district', 'localbody', 'bid'));
    }
...
//        getting id of inserted data
        if ($data) {
            ...
            return redirect()->route('personal_property.personal_create', compact('bid'));
        }

        return redirect()->route('personal_property.choose', compact('bid', 'poid', 'status'));
    ...
    public function corporate_property_create($bid)
    {
        ...
        return view('personal_property.corporate_create', compact('district','isin', 'localbody', 'ministry', 'department', 'bid'));
    }
...
//                getting inserted id
            ...
            if ($data) {
                ...
                Session::flash('danger', 'Property owner Creation Failed, Please Try Again.');
                return redirect()->route('personal_property.index', compact('bid'));
            }
...
//        ])->get();
        ...
        return view('personal_property.choose', compact('bid','jland','isin', 'status', 'property_owner', 'land_property', 'share_property', 'district', 'localbody', 'poid', 'pland', 'cland', 'pshare', 'cshare'));
    }
            ...
            if ($request->input('status') == 'personal') {
                if ($district_id[$i] && $local_body_id[$i] && $wardno[$i] && $kitta_no[$i] && $area[$i] && $malpot[$i]) {
                    ...
                    if ($result) {
                        ...
                        $result1 = PersonalLandBorrower::create($property);
                    }
                }
            }
            ...
            if ($request->input('status') == 'personal') {
                if ($dpid[$i] && $kitta[$i] && $isin[$i] && $share_type[$i]) {
                    ...
                    if ($result) {
                        ...
                        $result1 = PersonalShareBorrower::create($property);
                    }
                }
            }
//corporate
    ...
    public function land_edit($bid, $pid, $status)
    {
        ...
        return view('personal_property.edit_land', compact('bid', 'status', 'pid', 'land', 'localbody', 'district'));
    ...
    public function land_update(LandUpdateRequest $request)
    {
        ...
        if ($result) {
            ...
            return redirect()->route('personal_property.land_edit', compact('bid', 'poid', 'status'));
        }
        return redirect()->route('personal_property.choose', compact('bid', 'poid', 'status'));
    }
    ...
    public function land_assign($bid, $pid, $status)
    {
        ...
        if ($status == 'personal') {
            ...
            $check = PersonalLandBorrower::where([
            ...
            if (!$check) {
                $result = PersonalLandBorrower::create([
                    'borrower_id' => $bid,
                    'status' => '1',
    ...
    public function share_edit($bid, $pid, $status)
    {
        ...
        return view('personal_property.edit_share', compact('bid', 'isin','status', 'pid', 'share'));
...
//dd($request);
        ...
        if ($result) {
            ...
            Session::flash('danger', 'Failed to Edit Property');
            return redirect()->route('personal_property.share_edit', compact('bid', 'poid', 'status'));
        }
        return redirect()->route('personal_property.choose', compact('bid', 'poid', 'status'));
    }
    ...
    public function share_assign($bid, $pid, $status)
    {
        ...
        if ($status == 'personal') {
            ...
            $check = PersonalShareBorrower::where([
            ...
            if (!$check) {
                $result = PersonalShareBorrower::create([
                    'borrower_id' => $bid,
                    'status' => '1',
    ...
    public function share_borrower_edit($bid, $pid, $stat, $status)
    {
        ...
        return view('personal_property.edit_borrower_share', compact('bid','isin', 'status', 'stats', 'pid', 'share'));
    ...
    public function share_borrower_destroy($bid, $pid, $stat, $status)
    {
        ...
        if ($stat == 'personal') {
            $share = PersonalShare::find($pid);
            $poid = $share->property_owner_id;
            $pshare = PersonalShareBorrower::where([['personal_share_id', $pid], ['borrower_id', $bid]])->get();
            foreach ($pshare as $p) {
                try {
                    $result = $p->delete();
        ...
        if ($stat == 'corporate') {
            $share = CorporateShare::find($pid);
            $poid = $share->property_owner_id;
            $cshare = PersonalShareBorrower::where([['corporate_share_id', $pid], ['borrower_id', $bid]])->get();
            foreach ($cshare as $c) {
                try {
                    $result = $c->delete();
    ...
    public function land_borrower_edit($bid, $pid, $stat, $status)
    {
        ...
        return view('personal_property.edit_borrower_land', compact('bid', 'status', 'stats', 'pid', 'land', 'localbody', 'district'));
    ...
    public function land_borrower_destroy($bid, $pid, $stat, $status)
    {
        ...
        if ($stat == 'personal') {
            $land = PersonalLand::find($pid);
            $poid = $land->property_owner_id;
            $pland = PersonalLandBorrower::where([['personal_land_id', $pid], ['borrower_id', $bid]])->get();
            foreach ($pland as $p) {
                try {
                    $result = $p->delete();
        ...
        if ($stat == 'corporate') {
            $land = CorporateLand::find($pid);
            $poid = $land->property_owner_id;
            $cland = PersonalLandBorrower::where([['corporate_land_id', $pid], ['borrower_id', $bid]])->get();
            foreach ($cland as $c) {
                try {
                    $result = $c->delete();
...
//     for personal
            ...
            if ($model_number[$i] && $registration_number[$i] && $engine_number[$i] && $chassis_number[$i]) {
                ...
                $result = PersonalHirePurchase::create($data);
    ...
    public function vehicle_edit($bid, $id)
    {
        ...
        return view('personal_property.edit_vehicle', compact('bid', 'vehicle'));
    }
    ...
    public function share_front_edit($bid, $pid, $status)
    {
        ...
        return view('personal_property.edit_sharefront', compact('bid','isin', 'status', 'pid', 'share'));
    }
...
$borrower=PersonalBorrower::find($bid);
        ...
        return view('personal_property.edit_landfront', compact('bid', 'status', 'pid', 'land', 'localbody', 'district'));
...
//    joint front
    public function joint_land_front_edit($bid, $pid)
    {
        ...
        return view('personal_property.edit_jointlandfront', compact('bid','pid','land', 'localbody', 'district'));
    }

    public function joint_land_front_destroy($bid, $pid)
    {
        $result = false;

            $pland = JointLand::find($pid);
                try {
                    $result = $pland->delete();
    ...
    public function joint_land_front_update(LandUpdateRequest $request)
    {
        $result = false;
        $bid = $request->input('bid');
        $pid = $request->input('id');
