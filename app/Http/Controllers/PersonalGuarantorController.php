...
class PersonalGuarantorController extends Controller
{
    ...
    public function personal_guarantor_index($bid)
    {
...
//        $localbody = [];
        ...
        return view('personal_guarantor.index', compact('bid', 'personal_guarantor_list', 'corporate_guarantor_list', 'localbody', 'district', 'personal_borrower', 'personal_guarantor', 'personal_property_owner', 'corporate_borrower', 'corporate_guarantor', 'corporate_property_owner'));
    }
...
//                getting inserted id
                $gid = $data->id;
                $data1 = PersonalGuarantorBorrower::create([
                    'borrower_id' => $bid,
...
//                if name already exist in guarantor
                $check1 = PersonalGuarantorBorrower::where([
                    ['borrower_id', $bid],
...
//            if doesnot exist
                if (!$check1) {
                    ...
                    $data2 = PersonalGuarantorBorrower::create([
                        'borrower_id' => $bid,
                        'personal_guarantor_id' => $check->id,
                        'guarantor_type' => 'personal',
                        'status' => '1',
    ...
    {
        $borrower_check = PersonalBorrower::find($bid);
        $guarantor_check = PersonalGuarantor::find($id);
//            checking whether it exist or not

        $check = PersonalGuarantorBorrower::where([
            ['borrower_id', $bid],
            ['personal_guarantor_id', $id],
            ['guarantor_type', 'personal'],
        ])->first();

        if ($guarantor_check->citizenship_number == $borrower_check->citizenship_number) {
            ...
            //            if doesnot exist
            if (!$check) {
                $data = PersonalGuarantorBorrower::create([
                    'borrower_id' => $bid,
                    'personal_guarantor_id' => $id,
                    'guarantor_type' => 'personal',
                    'status' => '1',
    ...
    {
        $borrower_check = PersonalBorrower::find($bid);
        $property_owner_check = PersonalPropertyOwner::find($id);
        if ($property_owner_check->citizenship_number == $borrower_check->citizenship_number) {
            Session::flash('warning', 'Borrower cant be Own Guarantor');
            return redirect()->route('personal_guarantor.index', compact('bid'));
        } else {
...
//            if doesnot exist
    ...
    {
$status=null;
        $guarantor = PersonalGuarantorBorrower::find($gid);
        try {
...
//        local body
        ...
        return view('personal_guarantor.personal_create', compact('district', 'localbody', 'bid'));
    }

    public function personal_guarantor_store(PersonalCreateRequest $request, $bid)
    {
...
//        getting id of inserted data
    ...
    public function corporate_guarantor_create($bid)
    {
        $district = [];
        $corporate_borrower = CorporateBorrower::all();
        $dis = District::select('id', 'name')->get();
        ...
        return view('personal_guarantor.corporate_create', compact('corporate_borrower', 'district', 'localbody', 'ministry', 'department', 'bid'));
    }
...
//      dd($request);
            ...
            if ($data) {
                ...
                if ($data1) {
                    ...
                    if ($data2) {
                        ...
                        return redirect()->route('personal_guarantor.corporate_create');
        ...
        if ($data2) {
            $data3 = PersonalGuarantorBorrower::create([
                'borrower_id' => $bid,
            ...
            Session::flash('danger', 'Error in creating guarantor, Please Try again.');
            return redirect()->route('personal_guarantor.index');
        }
    }
}
